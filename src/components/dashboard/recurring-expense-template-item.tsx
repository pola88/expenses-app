...
import type { ExpenseTemplate } from '@/types/movement'
import { formatMoney } from '@/lib/money'
import { apiFetch } from '@/lib/fetch'

type Props = { template: ExpenseTemplate; onEdit: (t: ExpenseTemplate) => void }

export function RecurringExpenseTemplateItem({ template, onEdit }: Props) {
  const t = useTranslations('dashboard.recurringTemplate')
  const tCommon = useTranslations('common')
  const qc = useQueryClient()
  const [stopOpen, setStopOpen] = useState(false)

  const patch = (recurringActive: boolean) =>
    apiFetch(`/api/expenses/${template.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recurringActive }),
    })

  const stopMutation = useMutation({
    mutationFn: () => patch(false),
    onSuccess: () => {
  ...
  return (
    <>
      <div className={`flex items-center gap-3 py-3 ${!template.recurringActive ? 'opacity-50' : ''}`}>
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-red-50 text-lg">
